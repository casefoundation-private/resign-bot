...
const generateReviewConfig = (config) => {
  return new Promise((resolve,reject) => {
    ...
    for(var i = 0; i < (parseInt(process.env.REVIEW_CATEGORIES_COUNT) || 0); i++) {
      const prop = 'REVIEW_CATEGORY_' + i;
      config.review.categories[i] = {
        'prompt': process.env[prop],
        'labels': []
      };
      for(var j = 0; j < (parseInt(process.env['REVIEW_CATEGORY_' + i + '_LABELS_COUNT']) || parseInt(process.env.REVIEW_CATEGORY_LABELS_COUNT) || 0); j++) {
